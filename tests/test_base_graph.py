...
class TestGraph:
    ...
    def test_add_node_invalid_attributes_ignored(self):
        ...
        graph = BaseGraph(name="", value=0)
        graph.add_node(
            name="Valid", value=10, invalid_key="Ignored", wrong_type=["list"]
        )
    ...
    def test_modify_node(self):
        ...
        assert graph.nodes[uid].active is False  # Non modificato
...
class TestGraphMLSerialization:
    ...
    def test_export_graphml_creates_file(self):
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", delete=False, suffix=".graphml"
        ) as f:
            filepath = f.name
    ...
    def test_graphml_with_different_types(self):
        ...
        original = BaseGraph(text="", count=0, score=0.0, enabled=True)

        uid = original.add_node(text="Sample", count=42, score=98.5, enabled=False)
...
class TestDynamicOrdering:
    """Tests for register_ordering / _rebuild_ordering dynamic chain edges."""

    def test_register_ordering_invalid_key_raises(self):
        graph = BaseGraph(name="")
        with pytest.raises(KeyError, match="not in graph schema"):
            graph.register_ordering("nonexistent", "seq")

    def test_register_ordering_builds_initial_chain(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-03-01")
        b = graph.add_node(uid="b", date="2024-01-15")
        c = graph.add_node(uid="c", date="2024-02-10")
        graph.register_ordering("date", "seq")
        # Sorted ascending: b < c < a
        assert graph.has_edge(b, c, "seq")
        assert graph.has_edge(c, a, "seq")
        assert not graph.has_edge(a, b, "seq")

    def test_register_ordering_reverse(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-03-01")
        b = graph.add_node(uid="b", date="2024-01-15")
        c = graph.add_node(uid="c", date="2024-02-10")
        graph.register_ordering("date", "seq", reverse=True)
        # Descending: a > c > b
        assert graph.has_edge(a, c, "seq")
        assert graph.has_edge(c, b, "seq")
        assert not graph.has_edge(b, a, "seq")

    def test_add_node_triggers_rebuild(self):
        graph = BaseGraph(date="")
        b = graph.add_node(uid="b", date="2024-01-15")
        c = graph.add_node(uid="c", date="2024-02-10")
        graph.register_ordering("date", "seq")
        assert graph.has_edge(b, c, "seq")
        # Insert node that falls between b and c
        mid = graph.add_node(uid="mid", date="2024-01-20")
        assert graph.has_edge(b, mid, "seq")
        assert graph.has_edge(mid, c, "seq")
        assert not graph.has_edge(b, c, "seq")

    def test_add_node_without_key_goes_to_tail(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-01-01")
        b = graph.add_node(uid="b", date="2024-02-01")
        graph.register_ordering("date", "seq")
        # Add node without date
        z = graph.add_node(uid="z")
        assert graph.has_edge(b, z, "seq")
        assert not graph.has_edge(z, a, "seq")

    def test_del_node_triggers_rebuild(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-01-01")
        b = graph.add_node(uid="b", date="2024-02-01")
        c = graph.add_node(uid="c", date="2024-03-01")
        graph.register_ordering("date", "seq")
        # Chain: a -> b -> c
        graph.del_node(b)
        # Gap should be closed: a -> c
        assert graph.has_edge(a, c, "seq")
        assert not graph.has_edge(a, b, "seq")

    def test_modify_node_ordering_key_triggers_rebuild(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-01-01")
        b = graph.add_node(uid="b", date="2024-02-01")
        c = graph.add_node(uid="c", date="2024-03-01")
        graph.register_ordering("date", "seq")
        # Chain: a -> b -> c; move a to the end by giving it the latest date
        graph.modify_node(a, date="2024-12-01")
        assert graph.has_edge(b, c, "seq")
        assert graph.has_edge(c, a, "seq")
        assert not graph.has_edge(a, b, "seq")

    def test_modify_node_non_ordering_key_no_rebuild(self):
        graph = BaseGraph(date="", name="")
        a = graph.add_node(uid="a", date="2024-01-01")
        b = graph.add_node(uid="b", date="2024-02-01")
        graph.register_ordering("date", "seq")
        # Modifying an unrelated key should not change the chain
        graph.modify_node(a, name="updated")
        assert graph.has_edge(a, b, "seq")

    def test_multiple_orderings_on_same_graph(self):
        graph = BaseGraph(date="", priority=0)
        a = graph.add_node(uid="a", date="2024-03-01", priority=1)
        b = graph.add_node(uid="b", date="2024-01-15", priority=3)
        c = graph.add_node(uid="c", date="2024-02-10", priority=2)
        graph.register_ordering("date", "by_date")
        graph.register_ordering("priority", "by_priority")
        # by_date chain: b -> c -> a
        assert graph.has_edge(b, c, "by_date")
        assert graph.has_edge(c, a, "by_date")
        # by_priority chain: a(1) -> c(2) -> b(3)
        assert graph.has_edge(a, c, "by_priority")
        assert graph.has_edge(c, b, "by_priority")

    def test_nodes_without_key_stable_insertion_order(self):
        graph = BaseGraph(date="")
        x = graph.add_node(uid="x")  # inserted first
        y = graph.add_node(uid="y")  # second
        z = graph.add_node(uid="z")  # third
        graph.register_ordering("date", "seq")
        # All have no date: tail in insertion order x -> y -> z
        assert graph.has_edge(x, y, "seq")
        assert graph.has_edge(y, z, "seq")

    def test_chain_is_single_linked_list(self):
        graph = BaseGraph(date="")
        uids = [graph.add_node(date=f"2024-0{i+1}-01") for i in range(5)]
        graph.register_ordering("date", "seq")
        # Every node except the last has exactly one successor
        for uid in uids[:-1]:
            assert len(graph.get_neighbors(uid, "seq")) == 1
        # Every node except the first has exactly one predecessor
        for uid in uids[1:]:
            assert len(graph.get_predecessors(uid, "seq")) == 1
        # First has no predecessors, last has no successors
        assert len(graph.get_predecessors(uids[0], "seq")) == 0
        assert len(graph.get_neighbors(uids[-1], "seq")) == 0

    def test_to_dict_includes_orderings(self):
        graph = BaseGraph(date="")
        graph.add_node(uid="a", date="2024-01-01")
        graph.register_ordering("date", "seq")
        d = graph.to_dict()
        assert "orderings" in d
        assert d["orderings"] == {"seq": {"key": "date", "reverse": False}}

    def test_from_dict_restores_orderings_and_rebuilds(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-03-01")
        b = graph.add_node(uid="b", date="2024-01-15")
        graph.register_ordering("date", "seq")
        restored = BaseGraph.from_dict(graph.to_dict())
        assert restored._orderings == {"seq": {"key": "date", "reverse": False}}
        assert restored.has_edge(b, a, "seq")
        # New node added to restored graph should trigger rebuild
        c = restored.add_node(uid="c", date="2024-02-10")
        assert restored.has_edge(b, c, "seq")
        assert restored.has_edge(c, a, "seq")
    ...
    def test_from_dict_backward_compatible(self):
        ...
        d = graph.to_dict()
        del d["orderings"]
        restored = BaseGraph.from_dict(d)
        assert restored._orderings == {}

    def test_register_ordering_replaces_existing(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-03-01")
        b = graph.add_node(uid="b", date="2024-01-15")
        graph.register_ordering("date", "seq")
        assert graph.has_edge(b, a, "seq")
        # Re-register in reverse
        graph.register_ordering("date", "seq", reverse=True)
        assert graph._orderings["seq"]["reverse"] is True
        assert graph.has_edge(a, b, "seq")
        assert not graph.has_edge(b, a, "seq")

    def test_rebuild_wipes_old_edges_of_tipo(self):
        graph = BaseGraph(date="")
        a = graph.add_node(uid="a", date="2024-01-01")
        b = graph.add_node(uid="b", date="2024-02-01")
        # Manually add a backwards edge of the tipo we are about to register
        graph.add_edge(b, a, "seq")
        graph.register_ordering("date", "seq")
